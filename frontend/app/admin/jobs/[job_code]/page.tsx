...
export default function AdminJobDetail({ params }: {params: Promise<{job_code: string}>}) {
  const { job_code } = use(params); const router = useRouter(); const [job, setJob] = useState<Job>(); const [error, setError] = useState(""); const [saved, setSaved] = useState(""); const [busy, setBusy] = useState(false);
  ...
  async function deleteJob() { if (!confirm(`Xóa vĩnh viễn hồ sơ ${job_code} và toàn bộ tệp liên quan? Thao tác này không thể hoàn tác.`)) return; setBusy(true); setError(""); try { await api(`/admin/jobs/${job_code}`, {method:"DELETE"}); router.push("/admin"); router.refresh(); } catch (err) { setError(err instanceof Error ? err.message : "Không thể xóa hồ sơ"); setBusy(false); } }
