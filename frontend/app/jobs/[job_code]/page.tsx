  ...
  return <Shell wide><Link href="/" className="mb-4 inline-block text-sm text-slate-600">← Danh sách hồ sơ</Link>
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_340px] 2xl:grid-cols-[minmax(0,1fr)_380px]">
      ...
      <aside className="space-y-5 xl:sticky xl:top-5">
        ...
        <section className="card"><h2 className="font-bold">Tiến độ</h2><div className="mt-4 space-y-3">{steps.map((label, i) => <div key={label} className="flex items-center gap-3"><span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-xs font-bold ${i < current ? "border-emerald-600 bg-emerald-600 text-white" : i === current ? "border-brand bg-brand text-white" : "border-slate-300 text-slate-400"}`}>{i < current ? "✓" : i + 1}</span><span className={`text-sm ${i <= current ? "font-semibold" : "text-slate-400"}`}>{label}</span></div>)}</div></section>
